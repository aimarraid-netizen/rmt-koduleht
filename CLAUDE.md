...
- **Logod avalikult** (25.09.2026): kõik 25 faili (lisandusid tihe, läbipaistvad PNG-d, profiil, hele-taust-kitsas) → `https://rmtkonsult.ee/logo/<fail>`. Väljaspool meie lehte kasuta PNG-d (SVG `<text>` ei saa fonti). Tihe- ja profiil-SVG-del puudub `textLength`-parandus. `.gitattributes` export-ignore hoiab CLAUDE.md ja CNAME serverist eemal (git archive). Elamus Puhastuse logokomplekt (26 faili, originaal + lihtsustatud) → `https://rmtkonsult.ee/logo/elamus/<originaal|lihtsustatud>/<fail>` (Aimari valik 25.09.2026, sest elamuspuhastus.ee on veel Telias; go-live'il võib lisada ka elamuspuhastus.ee/logo/, aga need URL-id jäävad püsima).
